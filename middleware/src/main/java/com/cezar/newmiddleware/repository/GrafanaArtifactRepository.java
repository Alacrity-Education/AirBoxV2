...
public class GrafanaArtifactRepository {
    private static final String INSERT_QUERY  = """
            INSERT INTO airbox_grafana_artifacts(dashboard_uid, device_id, view, slug, folder_uid,
    ...
    // Newest row wins: during the uid-rename deploy window an orphaned st1-/st2- row and its
    // freshly synced abx-* replacement can briefly share a slug; the abx-* row is synced later.
    private static final String READ_BY_SLUG_QUERY = """
            SELECT * FROM airbox_grafana_artifacts WHERE slug = ? ORDER BY synced_at DESC
            """;

    private static final String UPDATE_QUERY = """
            UPDATE airbox_grafana_artifacts
            SET device_id = ?, view = ?, slug = ?, folder_uid = ?, access_token = ?,
    ...
    public void upsert(GrafanaArtifactDTO ga) {
        int rowsAffected = jdbcTemplate.update(UPDATE_QUERY, ga.deviceId(), ga.view(), ga.slug(),
        ...
        if(rowsAffected == 0) {
            jdbcTemplate.update(INSERT_QUERY, ga.dashboardUid(), ga.deviceId(),
                                ga.view(), ga.slug(), ga.folderUid(), ga.accessToken(),
    ...
    /** Resolve a public slug to its artifact row (newest wins on a transient duplicate). */
    public Optional<GrafanaArtifactDTO> findBySlug(String slug) {
        return jdbcTemplate.query(READ_BY_SLUG_QUERY, ARTIFACT_DTO_ROW_MAPPER, slug)
                .stream().findFirst();
    }
}
