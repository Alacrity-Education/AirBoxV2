...
- **AQI enrichment** (V9): EPA-style AQI computed at ingest into nullable `aqi` + `aqi_pollutant` — truncation → interpolation over breakpoint tables (registry covers all six EPA pollutants) → max sub-index. Fed by PM2.5/PM10 24 h trailing means and NOx as an NO2 1 h-mean ppb proxy (documented approximation). NULL unless ≥2 sub-indices incl. one PM (gate lowered from 3 on 2026-07-25 so real SEN66 devices qualify). A CUSTOM non-EPA CO2 sub-index (indoor-air-guidance breakpoints, 1 h mean) also participates. Historical rows recomputable via `infrastructure/scripts/backfill-aqi.py` (duplicates the formula — drift warnings in both sites).
- **Gas-index computation** (V10): SEN66 units may send `voc_raw`/`nox_raw` SGP41 ticks (0–65535) instead of the index fields; the ingest converts them via the Sensirion GasIndexAlgorithm port (`gasindex/`), storing the resulting `voc_index`/`nox_index`. Per-(device, VOC/NOX) algorithm state is Jackson-JSON in `airbox_gas_algorithm`, row-locked and upserted in the same transaction as the reading insert (dt from `updated_at`; 45 s uptime blackout returns 0). Explicit index values win over raw and do not advance state.
- **Migrations V4→V10**: artifacts table (V4), view-label renames (V5, V7), slugs (V6), station-view collapse (V8), AQI columns (V9), gas-algorithm state (V10). V3 seed is conditional on the `seed_mock_installations` Flyway placeholder.
