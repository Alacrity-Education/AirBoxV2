...
-- Seeding is gated by the Flyway placeholder ${seed_mock_installations} (true/false),
-- driven from application.properties (spring.flyway.placeholders.seed_mock_installations),
-- which is fed by MDW_SEED_MOCK_INSTALLATIONS / AIRBOX_MOCKS_ENABLED at deploy time.
-- When the placeholder resolves to "false" the WHERE clause is "WHERE false", so no rows
-- are inserted. The decision is made when this migration FIRST runs on a fresh database;
-- flipping the switch later does not retroactively add/remove rows.
INSERT INTO AIRBOX_INSTALLATIONS (device_id, apikey, owner_email, co_owner1_email, co_owner2_email, installation, notes)
SELECT device_id, apikey, owner_email, co_owner1_email, co_owner2_email, installation, notes
FROM (VALUES
...
) AS v(device_id, apikey, owner_email, co_owner1_email, co_owner2_email, installation, notes)
WHERE ${seed_mock_installations}
